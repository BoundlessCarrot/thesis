...
<<<<<<< HEAD
  ...
  global mask32;
  % G1 func
  g1 = bitxor(x(mod(1, 16)) + x(mod(9, 16)) + round_constants(constant_index), mask32);
  g1 = bitxor(bitshift(g1, 7), bitshift(g1, -25));
  g1 = bitxor(g1, bitand(x(mod(5, 16)) + g1, mask32));

  % G2 func
  g2 = bitxor(x(mod(5, 16)) + x(mod(1, 16)) + round_constants(constant_index), mask32);
  g2 = bitxor(bitshift(g2, 9), bitshift(g2, -23));
  g2 = bitxor(g2, bitand(x(mod(9, 16)) + g2, mask32));

  % Update state
  for j = 1:16
    x(j) = bitand(x(mod(j + 16, 16)) + g2 + bitxor(x(j), g1), mask32);
    c(j) = bitand(c(mod(j + 16, 16)) + g1 + bitxor(c(j), g2), mask32);
=======
  round_constants = [hex2dec('4D34D34D'), hex2dec('D34D34D3'), hex2dec('34D34D34'), hex2dec('4D34D34D')];
  mask32 = hex2dec('FFFFFFFF');
